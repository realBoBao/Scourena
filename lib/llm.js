 ...
 *   3. Vertex AI         (GOOGLE_APPLICATION_CREDENTIALS) — Gemini trên GCP * 
...
/** Tạo Vertex AI LLM instance (Gemini trên GCP) */
function createVertexLlm(opts = {}) {
  ...
  try {
    ...
    return vertex.getGenerativeModel({
      model: opts.model || 'gemini-1.5-flash-001',
    });
  } catch {
    return null;
  }
...
async function staticFallback(query, context = '') {
  ...
  // Mặc định
  return `Tôi đã nhận được câu hỏi: "${query.slice(0, 100)}". Hiện tại tôi đang ở chế độ offline (không có LLM API key). Các lệnh !quiz, !run, !debate, !animate vẫn hoạt động bình thường. Để kích hoạt AI đầy đủ, hãy thêm GEMINI_API_KEY hoặc OPENROUTER_API_KEY vào file .env.`;
}
...
export async function ask(query, opts = {}) {
  ...
  return Promise.race([
    ...
    if (err.message === 'LLM timeout') {
      ...
      return { answer: staticFallback(query), provider: 'static', model: 'timeout-fallback' };
    }
    throw err;
  });
}
...
async function _doAsk(query, { systemPrompt, temperature, maxTokens, opts }) {
  ...
  if (opts.provider === 'auto' || opts.provider === 'vertex' || !opts.provider) {
    const vertexLlm = createVertexLlm({ model: opts.model, temperature, maxTokens });
    if (vertexLlm) {
      try {
        const result = await vertexLlm.generateContent({
          contents: [{ role: 'user', parts: [{ text: query }] }],
        });
        const answer = result?.response?.candidates?.[0]?.content?.parts?.[0]?.text || '';
        if (answer.trim()) {
          return { answer: answer.trim(), provider: 'vertex', model: opts.model || 'gemini-1.5-flash' };
        }
      } catch (err) {
