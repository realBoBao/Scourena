...
async function getDb() {
  ...
  _dbPromise = (async () => {
    const db = await open({ filename: CACHE_DB, driver: null });
    await db.exec(`
...
// ── Tier 0: Jina Reader (AI-optimized content extraction) ────────────────────
const JINA_READER_URL = 'https://r.jina.ai/';

async function scrapeWithJina(url, timeout = 15000) {
  try {
...
// ── Main: scrapeUrl with all 5 tiers ──
/**
 ...
 * @param {boolean} [opts.useJina=true] — Allow Jina Reader (Tier 0, default true)
...
export async function scrapeUrl(url, opts = {}) {
  const { timeout = 10000, useCache = true, usePuppeteer = true, useDocker = true, useJina = true, cacheTtlHours = 168 } = opts;
  ...
  // Tier 0: Jina Reader (AI-optimized, bypass Cloudflare)
  if (useJina) {
    const jinaResult = await scrapeWithJina(url);
    if (jinaResult) {
      if (useCache) await cacheSet(url, jinaResult, '', cacheTtlHours);
      logger.info(`[WebScraper:T0] Jina Reader: ${url.slice(0, 80)} → ${jinaResult.length} chars`);
      return jinaResult;
    }
  }
