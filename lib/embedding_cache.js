...
export async function getCachedEmbedding(text) {
  ...
  // Tier 2: SQLite
  try {
    const row = getOne('SELECT embedding, created_at FROM embedding_cache WHERE text_hash = ?', [hash]);
    if (!row) return null;

    // TTL check
    if (Date.now() - row.created_at > DEFAULT_TTL_MS) {
      runQuery('DELETE FROM embedding_cache WHERE text_hash = ?', [hash]);
      return null;
    }

    // Update stats
    runQuery('UPDATE embedding_cache SET accessed_at = ?, hit_count = hit_count + 1 WHERE text_hash = ?', [Date.now(), hash]);
...
export async function setCachedEmbedding(text, embedding) {
  ...
  try {
    ...
    runDb(db, `CREATE TABLE IF NOT EXISTS embedding_cache (
    ...
    runDb(db, 'INSERT OR REPLACE INTO embedding_cache VALUES (?, ?, ?, ?, ?, ?, 1)',
      hash, text.slice(0, 80), buf, embedding.length, now, now);

    // Evict old
    const count = getDbRow(db, 'SELECT COUNT(*) as n FROM embedding_cache');
    if (count.n > DEFAULT_MAX) {
      const evict = Math.ceil(DEFAULT_MAX * 0.2);
      runDb(db, 'DELETE FROM embedding_cache WHERE text_hash IN (SELECT text_hash FROM embedding_cache ORDER BY accessed_at ASC LIMIT ?)', evict);
    }
