...
export async function embedText(text) {
  ...
  const localVec = localEmbedText(text);
  // Tier 1: Null Vector Validation — reject zero/near-zero vectors
  if (!localVec || localVec.length === 0) {
    throw new Error(`[Embedding] Local embedding failed: empty vector for "${text.slice(0, 50)}"`);
  }
  const magnitude = Math.sqrt(localVec.reduce((s, v) => s + v * v, 0));
  if (magnitude < 0.01) {
    throw new Error(`[Embedding] Local embedding failed: near-zero vector (mag=${magnitude.toFixed(4)}) for "${text.slice(0, 50)}"`);
  }
  return localVec;
}
...
export default { embedText, embedTextsBatch, cosineSimilarity, getEmbeddingCacheStats };
