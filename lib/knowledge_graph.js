 ...
 */
export async function searchEntities(query, type = null, limit = 10, domain = null) {
  const db = await getDb();
...
export async function getGraphStats() {
  ...
  const domains = db.prepare('SELECT domain, COUNT(*) as count FROM entities GROUP BY domain ORDER BY count DESC').all();
