...
// ── Singleton DB connection (uses node:sqlite directly for proper path) ──
let _dbPromise = null;
...
class MoodStateManager {
  ...
  async recordState(userId, { state, confidence, signals }) {
    ...
    if (db) {
      ...
      ).run(userId, state, confidence, JSON.stringify(signals), now.getHours(), now.getDay(), now.toISOString());
    }
    ...
    if (prev && prev.state !== state && db) {
      ...
      ).run(userId, prev.state, state, JSON.stringify(signals).slice(0, 200), now.toISOString());
  ...
  async getStateHistory(userId, days = 7) {
    const db = await _openDb();
    if (!db) return [];
    return db.prepare(`SELECT state, confidence, signals, hour_of_day, recorded_at FROM mood_history WHERE user_id = ? AND recorded_at > datetime('now', ?) ORDER BY recorded_at DESC`).all(userId, `-${days} days`);
  }
  ...
  async getDominantMood(userId, days = 7) {
    ...
    const rows = db.prepare(`SELECT state, COUNT(*) as count FROM mood_history WHERE user_id = ? AND recorded_at > datetime('now', ?) GROUP BY state ORDER BY count DESC LIMIT 1`).all(userId, `-${days} days`);
  ...
  async cleanup(daysToKeep = 90) {
    const db = await _openDb();
    if (!db) return;
    db.prepare("DELETE FROM mood_history WHERE recorded_at < datetime('now', ?)").run(`-${daysToKeep} days`);
    db.prepare("DELETE FROM mood_transitions WHERE recorded_at < datetime('now', ?)").run(`-${daysToKeep} days`);
