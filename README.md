...
- **🎙️ Voice Chat** — Tham gia voice channel, nghe & trả lời bằng giọng Việt (TTS)
- **📚 Study Mode** — `!voice study` → bot im lặng khi bạn học, chỉ nghe wake word "Serena"
- **🗣️ Voice Commands** — `!voice join/leave/study/stop` điều khiển voice channel
...
| **VoiceAgent** | Voice channel + whisper.cpp + TTS (Edge/ElevenLabs) + VAD + Study Mode |
...
!voice study                → Chế độ học (im lặng, chỉ nghe wake word "Serena")
!voice stop                 → Tắt chế độ học
!voice + audio              → Transcribe voice message (whisper.cpp)
```

**Voice Channel:** Serena tham gia voice channel, nghe bạn nói và trả lời bằng giọng Việt (Edge TTS miễn phí).
**Study Mode:** `!voice study` → Serena im lặng khi bạn học, chỉ lên tiếng khi gọi "Serena".
**PNGTuber:** Mở `http://<server>:3005/pngtuber` trên tablet/phone để xem avatar.
